...
import BoxManStage, { type Kp3dDoc } from './BoxManStage'
...
export default function Preview() {
  ...
  const [skinMode, setSkinMode] = useState<'off' | '3d' | '2d'>('off') // box-man skin: 3d cuboids (default) or 2d flat boxes from the SAM rig (experiments/box-man)
  const [s3body3d, setS3body3d] = useState<Kp3dDoc | null>(null) // current clip's 3D SAM keypoints (for the 3d box-man skin)
  const [shadow, setShadow] = useState(true) // contact shadow under her feet, on by default
  // Map-proofing view: see the baked depth/normal sidecars directly. plain → depth →
  // normal → relight → plain. relight is the "does the normal actually light right" check.
  const [view, setView] = useState<'plain' | 'depth' | 'normal' | 'relight'>('plain')
  const [lightDir, setLightDir] = useState<[number, number, number]>([0.4, -0.5, 0.75]) // relight light, tuned by dragging
  ...
  useEffect(() => {
    ...
    fetch(`/contents/${current.key.replace(/\.mp4$/, '.bizarre.json')}`)
  ...
  // The 3D box-man skin needs pred_keypoints_3d (a separate, larger sidecar) — fetch it
  // only when that skin is active, on clip change.
  useEffect(() => {
    if (skinMode !== '3d' || !current) {
      setS3body3d(null)
      return
    }
    let cancelled = false
    setS3body3d(null)
    fetch(`/contents/${current.key.replace(/\.mp4$/, '.s3body3d.json')}`)
      .then((r) => (r.ok ? (r.json() as Promise<Kp3dDoc>) : null))
      .then((d) => !cancelled && setS3body3d(d))
      .catch(() => !cancelled && setS3body3d(null))
    return () => {
      cancelled = true
    }
  }, [current?.key, skinMode])
    ...
    >
      ...
      {current && (
        <div ref={stageWrapRef} style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: barH }}>
          ...
          {/* Relight: drag anywhere on the stage to aim the light. Captures pointer so it
              doesn't fight zoom/scrub; only active in the relight view. */}
          {view === 'relight' && (
            <div
              onPointerDown={(e) => {
                e.stopPropagation()
              ...
              onPointerMove={(e) => {
                ...
                const mx = ((e.clientX - r.left) / r.width - 0.5) * 2 // -1..1 (right +)
                const my = ((e.clientY - r.top) / r.height - 0.5) * 2 // -1..1 (down +)
                setLightDir([mx * 1.6, my * 1.6, 0.45])
              }}
        ...
        >
          ...
          <button
            onClick={() =>
              setView((v) => (v === 'plain' ? 'depth' : v === 'depth' ? 'normal' : v === 'normal' ? 'relight' : 'plain'))
            }
            title="map proofing: plain → depth → normal → relight (relight: drag stage to aim light)"
            style={pill(view !== 'plain')}
          >
            {view === 'plain' ? '○ maps' : `◉ ${view}`}
            {view === 'relight' && <span style={{ opacity: 0.8 }}> · drag to aim</span>}
          </button>
          <button
          ...
          <button
            onClick={() => setSkinMode((s) => (s === 'off' ? '3d' : s === '3d' ? '2d' : 'off'))}
            title="box-man skin: 3d cuboids → 2d flat boxes → off (from the SAM rig)"
            style={pill(skinMode !== 'off')}
          >
            {skinMode === 'off' ? '○ skin' : skinMode === '3d' ? '◉ skin · 3d' : '◉ skin · 2d'}
            {skinMode === '3d' && !s3body3d && current && <span style={{ opacity: 0.8 }}>· no data</span>}
            {skinMode === '2d' && !s3body && current && <span style={{ opacity: 0.8 }}>· no data</span>}
          </button>
        </div>
