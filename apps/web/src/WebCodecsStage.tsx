...
// Map-proofing views: nt = .normal.mp4, dt = .depth.mp4 (both non-stacked, char-only).
// uView: 0 plain · 1 depth (grayscale) · 2 normal (raw RGB) · 3 relight (Lambert).
// uLight/uAmbient/uStrength drive the relight view.
uniform sampler2D nt;uniform sampler2D dt;uniform float uView;uniform vec3 uLight;uniform float uAmbient;uniform float uStrength;
...
void main(){
  ...
  if(uView>2.5){ // relight
    vec3 nn=texture2D(nt,vec2(u.x,u.y)).rgb*2.0-1.0;
    nn.y=-nn.y; // NormalCrafter Y is +up; screen/light Y is +down — align them
    nn=normalize(nn);
    float ndl=max(dot(nn,normalize(uLight)),0.0);
    rgb*=uAmbient+(1.0-uAmbient)*ndl*uStrength;
  } else if(uView>1.5){ // normal map, raw
    rgb=texture2D(nt,vec2(u.x,u.y)).rgb;
  } else if(uView>0.5){ // depth map, grayscale
    rgb=vec3(texture2D(dt,vec2(u.x,u.y)).r);
  }
...
type Props = {
  ...
  skin?: 'off' | 'box' // box-man skin: render the SAM rig as flat boxes + hide the video
  showFace?: boolean
  showShadow?: boolean
  view?: 'plain' | 'depth' | 'normal' | 'relight' // map-proofing view (loads .depth/.normal sidecars)
  lightDir?: [number, number, number] // relight light direction (tune by dragging)
  ambient?: number // floor brightness when a surface faces away (0..1)
  strength?: number // diffuse gain
  fps?: number
  scrub?: number | null
...
export default function WebCodecsStage({
  ...
  const cur = useRef({ scale, anchor, baseline, zoom, feather, mouth, mouthMode, mouthMargin, pose, s3body, face, showOverlay, overlaySource, skin, showFace, showShadow, view, lightDir, ambient, strength, fps })
  cur.current = { scale, anchor, baseline, zoom, feather, mouth, mouthMode, mouthMargin, pose, s3body, face, showOverlay, overlaySource, skin, showFace, showShadow, view, lightDir, ambient, strength, fps }
  ...
  useEffect(() => {
    ...
    const tex = mkTex() // unit 0: stacked RGB+alpha
    const normTex = mkTex() // unit 1: .normal.mp4 (non-stacked)
    const depthTex = mkTex() // unit 2: .depth.mp4 (non-stacked)
    ...
    const uNt = U('nt'), uDt = U('dt'), uView = U('uView'), uLight = U('uLight'), uAmbient = U('uAmbient'), uStrength = U('uStrength')
    gl.uniform1i(uT, 0)
    gl.uniform1i(uNt, 1)
    gl.uniform1i(uDt, 2)
    ...
    let normClip: StreamingClip | null = null // .normal.mp4 twin, frame-synced by index
    let depthClip: StreamingClip | null = null // .depth.mp4 twin, frame-synced by index
    let cancelled = false
    let raf = 0
    ...
    const draw = (now: number) => {
      ...
      // Map-proofing twins, locked to the SAME idx as the picture (1:1 by construction).
      // normal feeds the normal/relight views; depth feeds the depth view. Fall back to
      // the plain view (0) if the needed sidecar hasn't decoded a frame yet.
      const wantNorm = c.view === 'normal' || c.view === 'relight'
      const wantDepth = c.view === 'depth'
      let viewN = 0
      if (wantNorm && normClip) {
        const ng = normClip.frameAt(idx)
        if (ng) {
          gl.activeTexture(gl.TEXTURE1)
          gl.bindTexture(gl.TEXTURE_2D, normTex)
          gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, ng.frame)
          viewN = c.view === 'relight' ? 3 : 2
        }
      } else if (wantDepth && depthClip) {
        const dg = depthClip.frameAt(idx)
        if (dg) {
          gl.activeTexture(gl.TEXTURE2)
          gl.bindTexture(gl.TEXTURE_2D, depthTex)
          gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, dg.frame)
          viewN = 1
        }
      }
      gl.uniform1f(uView, viewN)
      gl.uniform3fv(uLight, c.lightDir)
      gl.uniform1f(uAmbient, c.ambient)
      ...
      gl.uniform1f(uMouthFeather, 0.0055) // ~3.5px outward feather (frame is 640px)
      ...
      if (octx) {
        ...
        if (c.skin === 'box') {
          ...
          if (skp) drawSamBoxes(octx, skp, proj)
        }
        if (c.showOverlay) {
    ...
    // Map-proofing sidecars, decoded in parallel; their view stays plain until ready.
    const sidecar = (suffix: string, set: (c: StreamingClip) => void) => {
      const s = src.replace(/\.mp4(\?.*)?$/, `${suffix}$1`)
      if (s === src) return
      StreamingClip.create(s, cur.current.fps)
        .then((c) => { if (cancelled) c.close(); else set(c) })
        .catch((e) => console.warn('WebCodecsStage: no sidecar', s, e?.message || e))
    }
    sidecar('.normal.mp4', (c) => (normClip = c))
    sidecar('.depth.mp4', (c) => (depthClip = c))
  ...
  return (
    <div style={{ position: 'relative', ...style }}>
      ...
      <canvas ref={canvasRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', display: 'block', opacity: skin === 'box' ? 0 : 1 }} />
