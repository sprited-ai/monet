...
export const SAM_EDGES: [number, number][] = [
...
// box-man skin (experiments/box-man) — draw the SAM rig as flat blocky boxes instead of
// the x-ray skeleton: a filled rect per bone + a head box with dot eyes. Same `kp`, same
// `project` as drawSamOverlay, so it's frame-locked. Sharp corners = the "kitbash cardboard"
// look. Fingers (21–40 / 42–61) are skipped — a blocky puppet doesn't need 40 knuckle boxes.
const BOX_FILL = '#efe4cf' // warm cardboard
const BOX_INK = '#2f2820' // dark ink outline

function boxSeg(ctx: CanvasRenderingContext2D, A: [number, number], B: [number, number], th: number) {
  const dx = B[0] - A[0], dy = B[1] - A[1]
  const len = Math.hypot(dx, dy)
  ctx.save()
  ctx.translate((A[0] + B[0]) / 2, (A[1] + B[1]) / 2)
  ctx.rotate(Math.atan2(dy, dx))
  const w = len + th * 0.6, h = th
  ctx.fillRect(-w / 2, -h / 2, w, h)
  ctx.strokeRect(-w / 2, -h / 2, w, h)
  ctx.restore()
}

export function drawSamBoxes(
  ctx: CanvasRenderingContext2D,
  kp: [number, number][],
  project: (ux: number, uy: number) => [number, number],
) {
  ...
  const shL = P(5), shR = P(6), hipL = P(9), hipR = P(10)
  const unit = shL && shR ? Math.max(10, Math.hypot(shR[0] - shL[0], shR[1] - shL[1])) : 40
  const limbTh = unit * 0.42, armTh = unit * 0.34
  // torso slab (shoulders → hips)
  if (shL && shR && hipL && hipR) {
    ctx.beginPath()
    ctx.moveTo(shL[0], shL[1]); ctx.lineTo(shR[0], shR[1])
    ctx.lineTo(hipR[0], hipR[1]); ctx.lineTo(hipL[0], hipL[1])
    ctx.closePath(); ctx.fill(); ctx.stroke()
  }
  // limb bones (skip finger joints + intra-head edges)
  const finger = (i: number) => (i >= 21 && i <= 40) || (i >= 42 && i <= 61)
  const head = new Set([0, 1, 2, 3, 4])
  const armIdx = new Set([7, 8, 41, 62])
  for (let e = 0; e < SAM_EDGES.length; e++) {
    const [a, b] = SAM_EDGES[e]
    if (finger(a) || finger(b)) continue
    if (head.has(a) && head.has(b)) continue
    const A = P(a), B = P(b)
    if (!A || !B) continue
    boxSeg(ctx, A, B, armIdx.has(a) || armIdx.has(b) ? armTh : limbTh)
  }
  // head box + dot eyes (the spark of life)
  const hpts = [0, 1, 2, 3, 4].map(P).filter(Boolean) as [number, number][]
  if (hpts.length) {
    const xs = hpts.map((p) => p[0]), ys = hpts.map((p) => p[1])
    const cx = (Math.min(...xs) + Math.max(...xs)) / 2
    const cy = (Math.min(...ys) + Math.max(...ys)) / 2
    const hw = Math.max(unit * 0.6, (Math.max(...xs) - Math.min(...xs)) * 0.7 + unit * 0.3)
    const hh = hw * 1.05
    ctx.fillRect(cx - hw, cy - hh, hw * 2, hh * 1.9)
    ctx.strokeRect(cx - hw, cy - hh, hw * 2, hh * 1.9)
    ctx.fillStyle = BOX_INK
    for (const eye of [P(1), P(2)]) {
      if (!eye) continue
      ctx.beginPath(); ctx.arc(eye[0], eye[1], Math.max(2, unit * 0.1), 0, 7); ctx.fill()
    }
    ctx.fillStyle = BOX_FILL
  }
}
