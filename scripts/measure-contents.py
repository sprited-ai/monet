...
def main():
    ...
    files = sorted(p for p in base.iterdir()
                   ...
                   and ".depth." not in p.name       # skip colocated depth sidecars
                   and ".normal." not in p.name      # skip colocated normal sidecars
                   and "mouth-atlas" not in p.name)  # skip the lip-sync mouth atlas + its label (tooling, not content)
    items, framings = {}, {}
    ...
    if fjson.exists():
        try:
            ...
            prev = {}
    # `mouthRig` on each animation is hand-owned (like framings `origin`): true = always rig,
    # "talk" = native baked mouth + rig only while speaking, false = never. Seeded "talk" by
    # default ("eat"/"drink"/"turn"/back-facing clips → false), then preserved across re-measures
    # so a hand-set value is never clobbered. See [[framings-origins-hand-owned]].
    prev_items = {}
    ijson = CONTENTS / "index.json"
    if ijson.exists():
        try:
            prev_items = json.loads(ijson.read_text()).get("items", {})
        except Exception:
            prev_items = {}
    ...
    for p in files:
        ...
        try:
            if p.suffix.lower() in ANIM_EXT:
                ...
                prevv = prev_items.get(name, {})
                entry["mouthRig"] = prevv["mouthRig"] if "mouthRig" in prevv else (
                    False if any(k in name for k in ("eat", "drink", "turn", "back-to-front", "back-walk")) else "talk")
            else:
