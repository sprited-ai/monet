...
in `inspirations/`, `styles/`, `archived/`. Paper notes live in `papers/`.

## Papers

- [Anime-Ready (ICLR 2026)](papers/anime-ready-iclr2026.md) — text/image → fully
  rigged anime 3D char via **Anime-SMPL** (PCA over 20k chars) + MoE garment DiT
  with body-latent geometry prior + per-component texturing. Direct counterpoint to
  our `sam-3d-body-not-for-monet` path; assets are DreamTech-proprietary.
