...
| | |
|---|---|
| reference | `inputs/monet_ref_idle1.webp` — frame 0 of `monet-idle-1`, color×alpha flattened on white (640×640) |
...
**Result** (`out/out_montage.webp`): the idle reference performs the driving motion —
...
The fast-motion (walk/run) frames read soft. Diagnosis from `out/run_sequence_orig.webp`:
...
`out/upscale_compare_run_frame.webp` (left 640 nearest, right 1280) — clearly crisper
...
Zoomed leg/shoe compare `out/compare_legs_6vs25.webp` (left 6-step, right 25-step): the
...
(`out/final_25step_upscaled_montage.webp`). Sharp source × 1280 anime upscale.
...
**Result** (`out/e2_human_vs_monet.webp` top=human / bottom=Monet; `out/e2_monet_montage.webp`):
...
`out/latency_320_vs_640.webp`: 320px (8 s) vs 640px (28 s) — 320 is softer but **still
...
no halo, white socks/light dress preserved (`out/matte_transparency_check.webp`).
...
inputs/monet_ref_idle1.webp          E1 reference still (clean Monet on white)
inputs/drive_preview.webp            4 frames of the driving clip (idle→wave→walk)
out/scail2_monet_e1_00001.mp4       E1 result (640×640×81, 16fps)
out/out_montage.webp                 6 frames of the result — the headline
```
