...
#
# Paths are env-overridable (gen-sidecars.sh sets them). Existing <clip>.s3body.json
# are SKIPPED, never overwritten — set FORCE=1 to regenerate.
#   S3BODY_NPZ  (default experiments/sam3d-body/out)   CONTENTS (default contents/monet)
import numpy as np, glob, os, json

SRC = os.environ.get("S3BODY_NPZ", "experiments/sam3d-body/out")
DEST = os.environ.get("CONTENTS", "contents/monet")
FORCE = os.environ.get("FORCE", "") not in ("", "0")
wrote = skipped = 0
for f in sorted(glob.glob(SRC + "/*.npz")):
    stem = os.path.basename(f)[:-4]
    outp = os.path.join(DEST, stem + ".s3body.json")
    ...
    wrote += 1
print(f"s3body.json: wrote {wrote}, skipped {skipped} (existing){' [FORCE]' if FORCE else ''}")
